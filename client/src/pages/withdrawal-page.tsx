...
export default function WithdrawalPage() {
  ...
  // Withdrawal form with direct state
  ...
  const onWithdrawalSubmit = (formData: WithdrawalFormValues) => {
    if (!user) return;

    // Use direct state values rather than form values
    const data = {
    ...
    if (!account?.balance || amountValue > account.balance) {
    ...
    if (!withdrawalCode || account.withdrawalCode !== withdrawalCode) {
    ...
    const withdrawalData = {
      amount: Number(amount) || 0, // Ensure amount is a number and never undefined
      withdrawalCode: withdrawalCode,
      userId: user.id,
    ...
    <div className="container max-w-md mx-auto p-4 h-full">
      ...
      <Card className="mb-6 overflow-hidden">
        <div 
          className="p-6" 
          style={{ 
            background: "var(--withdrawal-balance-card-gradient, linear-gradient(to right, var(--primary-color), var(--primary-dark)))",
            color: "var(--withdrawal-balance-text-color, #ffffff)"
          }}
        >
          <div className="flex justify-between items-center">
            <div>
              <p className="text-sm font-medium opacity-90 mb-2">ยอดเงินในบัญชี</p>
              {accountLoading ? (
      ...
      <div className="mb-6 rounded-xl overflow-hidden shadow-sm">
        <div className="grid grid-cols-2 bg-white/80 backdrop-blur-sm">
          <button
            ...
            style={{ 
              color: activeTab === "form" 
                ? "var(--withdrawal-tab-active-color, var(--primary-color))" 
                : "var(--withdrawal-tab-text-color, #4b5563)"
            }}
          ...
          >
            ...
            {activeTab === "form" && (
              <div 
                ...
                style={{ 
                  background: "var(--withdrawal-tab-active-color, var(--primary-color))"
                }}
              ></div>
            )}
          </button>
      ...
      {activeTab === "form" ? (
        <Card className="shadow-lg rounded-xl overflow-hidden">
          <CardHeader style={{ 
            background: !hasBankAccount 
              ? "var(--withdrawal-bank-form-header-gradient, linear-gradient(to right, #e6f7f7, #e6f0ff))" 
              : "var(--withdrawal-withdraw-form-header-gradient, linear-gradient(to right, #e6f7f7, #e6f0ff))"
           }}>
            <CardTitle className="text-lg flex items-center">
              {!hasBankAccount ? (
                ...
                <>
                  <Banknote className="mr-2 h-5 w-5 text-teal-600" />
              ...
              <Form {...bankAccountForm}>
                <form onSubmit={bankAccountForm.handleSubmit(onBankAccountSubmit)} className="space-y-6">
                  ...
                  <div className="pt-6">
                    <Button
                      type="submit"
                      className="w-full font-medium py-6 rounded-xl shadow-md transition-all duration-200 hover:shadow-lg"
                      style={{ 
                        background: "var(--withdrawal-bank-button-gradient, linear-gradient(to right, #3b82f6, #2563eb))",
                        color: "var(--withdrawal-bank-button-text-color, #ffffff)"
                      }}
                    >
                      <CreditCard className="h-5 w-5 mr-2" />
                      <span className="text-base">บันทึกข้อมูลบัญชี</span>
                    </Button>
                  </div>
                </form>
              </Form>
              ...
              <Form {...withdrawalForm}>
                <form onSubmit={withdrawalForm.handleSubmit(onWithdrawalSubmit)} className="space-y-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="relative mt-1.5">
                        ...
                        <input
                          id="amount"
                          type="number"
                          className="w-full pl-8 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                          placeholder="0.00"
                          value={amount}
                          onChange={(e) => {
                            const value = e.target.value;
                            setAmount(value);
                            // Always provide a number for React Hook Form
                            withdrawalForm.setValue("amount", value === "" ? 0 : Number(value));
                          }}
                        />
                      </div>
                    ...
                    <div>
                      <label htmlFor="withdrawalCode" className="text-sm font-medium">รหัสอนุมัติถอนเงิน</label>
                      <div className="relative mt-1.5">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">
                        ...
                        <input
                          ...
                          type="text"
                          className="w-full pl-8 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                          placeholder="ระบุรหัสอนุมัติของคุณ"
                          value={withdrawalCode}
                          onChange={(e) => {
                            const value = e.target.value;
                            setWithdrawalCode(value);
                            withdrawalForm.setValue("withdrawalCode", value);
                          }}
                        />
                      </div>
                      <p className="text-sm text-muted-foreground mt-1.5">
                        รหัสอนุมัติจากแอดมิน
                      </p>
                      {withdrawalForm.formState.errors.withdrawalCode && (
                        <p className="text-sm font-medium text-destructive mt-1.5">
                          {withdrawalForm.formState.errors.withdrawalCode.message}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="pt-6">
                    <Button
                      ...
                      className="w-full font-medium py-6 rounded-xl shadow-md transition-all duration-200 hover:shadow-lg"
                      style={{ 
                        background: "var(--withdrawal-withdraw-button-gradient, linear-gradient(to right, #10b981, #059669))",
                        color: "var(--withdrawal-withdraw-button-text-color, #ffffff)"
                      }}
        ...
        <Card className="shadow-lg rounded-xl overflow-hidden">
          <CardHeader style={{ 
            background: "var(--withdrawal-bank-form-header-gradient, linear-gradient(to right, #e6f7f7, #e6f0ff))"
          }}>
